...
if __name__ == '__main__':
    date = input("for FCBA: input a day to show schedule on that date (mm/dd/yyyy)\n")
    month, day, year = date.split('/')
    file = open("logs/[FAM] [" + year + "_" + month + "].txt", "r", errors='replace')
    ...
    for line in lines:
        ...
        if log_type == "PGRFAMCHA":
            showtime_year = "20" + log_date[10:12]
            showtime_month = getMonth(log_date[12:14])
            showtime_day = log_date[14:16]

            showtime_time = log_date[16:18] + log_date[18:20]
            show_name = log_date[34:]
